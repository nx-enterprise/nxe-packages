...
export default async function (host: Tree, options: AppGeneratorSchema) {
  ...
  addProjectConfiguration(host, normalizedOptions.projectName, {
    ...
    targets: {
      serve: {
        executor: '@nx-enterprise/nxe-python-fastapi:serve',
      },
      test: {
        executor: '@nx-enterprise/nxe-python-fastapi:test',
        outputs: [joinPathFragments('coverage', projectRoot)],
      },
      lint: {
        executor: '@nx-enterprise/nxe-python-fastapi:lint',
      },
      format: {
        executor: '@nx-enterprise/nxe-python-fastapi:format',
      },
    },
